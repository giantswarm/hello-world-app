...
| nodeSelector | object | `{}` | Node selector for pod scheduling. Merged with `architecture`. Setting `kubernetes.io/arch` here to something other than `architecture` is a contradiction and fails the render, so set only one of the two. |
